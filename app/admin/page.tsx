...
export default function AdminPage() {
  ...
  const [selectedGame, setSelectedGame] = useState<'opposites' | 'verb_past' | 'prepositions' | null>(null);
  ...
  async function load(currentUser: User | null) {
    ...
    const [{ data: admin }, { data: content }, { data: verbContent }, { data: prepositionContent }] = await Promise.all([
    ...
    setIsAdmin(!!admin); setItems((content ?? []) as GameItem[]); setVerbs((verbContent ?? []) as VerbGameItem[]); setPrepositions((prepositionContent ?? []) as PrepositionGameItem[]); setLoading(false);
  }
