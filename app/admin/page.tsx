...
import type { GameItem, PrepositionGameItem, VerbGameItem } from '@/lib/game-content';
...
type CatalogGame={game_key:GameKey;title:string;category:string;levels:string[];active:boolean;sort_order:number};
...
export default function AdminPage() {
  ...
  async function load(currentUser: User | null) {
    ...
    const [{ data: admin }, { data: content }, { data: verbContent }, { data: prepositionContent }, { data: catalog }] = await Promise.all([
      ...
      supabase.from('preposition_game_content').select('*').eq('level', 'B1').order('sort_order'),
      supabase.from('practice_games').select('*').order('sort_order'),
    ]);
    setIsAdmin(!!admin); setItems((content ?? []) as GameItem[]); setVerbs((verbContent ?? []) as VerbGameItem[]); setPrepositions((prepositionContent ?? []) as PrepositionGameItem[]); setGameCatalog((catalog??[]) as CatalogGame[]); setLoading(false);
  }
  function chooseGame(key:GameKey){setSelectedGame(key);setGameLevels(gameCatalog.find(game=>game.game_key===key)?.levels??[]);setMessage('');setLevelMessage('');}
  async function saveGameLevels(){if(!selectedGame||!gameLevels.length){setLevelMessage('Select at least one language level for this game.');return;}const{error}=await supabase.from('practice_games').update({levels:gameLevels}).eq('game_key',selectedGame);if(error){setLevelMessage(error.message);return;}await load(user);setLevelMessage('Game levels saved. Battle setup now uses this availability.');}
